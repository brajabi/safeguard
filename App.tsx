...
function Dashboard() {
  ...
  const assessment =
    tunnel.available && tunnel.active
      ? {
          title: "Possible VPN connection",
          detail:
            "An active tunnel was found on this device. VPNs and some system services use tunnels.",
          tone: "neutral",
        }
      : ip
        ? assessConnection(ip, zone)
        : null;
  ...
  return (
    <SafeAreaView style={s.safe} edges={["top", "left", "right"]}>
      ...
      >
        ...
        <View style={s.intro}>
          <Eyebrow>YOUR DIGITAL FOOTPRINT</Eyebrow>
          <Text style={s.title}>Where you appear.</Text>
          <Text style={s.subtitle}>
            A little clarity about your connection.
          </Text>
        </View>
        <View style={s.summary}>
          <View style={s.summaryIcon}>
            {loading ? (
              ...
              <Icon
                name={assessment?.tone === "warning" ? "alert-circle" : "globe"}
                size={23}
              />
            )}
          </View>
          <View style={{ flex: 1 }}>
            <Text style={s.summaryTitle}>
              {loading
                ...
                : (assessment?.title ?? "Connection unavailable")}
            </Text>
            <Text style={s.summaryText}>
              {loading
                ? "Reading your public IP and network signals."
                : (assessment?.detail ??
                  "Your device time zone is available below.")}
            </Text>
          </View>
        </View>
        {!!error && (
        ...
        <View style={s.card}>
          ...
          <View style={s.cardNote}>
            ...
            <Text style={s.note}>
              IP location is approximate. IPv6 availability is not a leak test.
            </Text>
          </View>
        </View>
        <View style={s.sectionHeading}>
        ...
        <View style={s.card}>
          <View style={s.segment}>
            ...
            <Pressable
              ...
              style={[s.segmentItem, selected === "device" && s.segmentActive]}
            >
              <Icon
                name="navigation"
                size={15}
          ...
          {device && ip && (
            <Text style={s.distance}>
              Your IP appears{" "}
              {Math.round(distanceKm(device, ip)).toLocaleString()} km from your
              device. This alone doesn’t prove VPN use.
            </Text>
          )}
        </View>
        <View style={s.sectionHeading}>
          ...
          >
            <Text
              style={[s.pillText, match === "different" && { color: C.amber }]}
            >
              {match === "match"
                ? "In sync"
                : match === "different"
        ...
        <View style={s.card}>
          ...
          <Text style={s.note}>
            ...
            rule out a VPN. Free IP lookups do not include VPN database checks,
            so those signals may be unknown.
          </Text>
        </View>
        <Button
          variant="outline"
        ...
        <SafeAreaView style={s.safe}>
          <ScrollView contentContainerStyle={s.modal}>
            ...
            <Text style={s.modalBody}>
              ...
              services. The native map provider receives map-view requests.
              There is no account, analytics, background tracking, or saved
              location history.
            </Text>
